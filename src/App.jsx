...
import { applySerial, buildSerialMap, normTurbine, oggi, parseText, splitName } from './lib/parser.js';
import { appendRows, deleteTurbina, getArchive, getTurbine, ocrImage, saveTurbina } from './lib/gas.js';
...
export default function App() {
  ...
  async function loadArchive(silent) {
    ...
    try {
      ...
      if (!silent) toast("Impossibile leggere l'archivio: " + e.message, 'err');
    } finally {
  ...
  async function processAll() {
    ...
    for (let i = 0; i < total; i++) {
      setProgress({ pct: Math.round((i / total) * 100), txt: `Leggo documento ${i + 1} di ${total}…` });
      let r;
      try {
    ...
    setQueue([]);
    setProcessing(false);
    toast(`${newRows.length} document${newRows.length === 1 ? 'o' : 'i'} pronti da verificare`, 'ok');
  }
  ...
  function addManualRow() {
    setRows((rs) => [...rs, { _id: nextId(), type: 'classic', dn: '', van: '', turbine: '', activity: '', sp: '', date: oggi(), wtg: '', _turbStatus: 'ok' }]);
  }
  ...
  async function confirmRows() {
    ...
    try {
      ...
      const payload = rows.map((r) => ({ date: r.date, dn: r.dn, van: r.van, wtg: r.wtg, turbine: r.turbine, activity: r.activity, sp: r.sp }));
      const j = await appendRows(warehouse, payload);
      if (j && j.ok) {
        toast(`Confermate ${j.inserted} righe → ${WH_LABELS[warehouse]}`, 'ok');
        const miss = [];
        if (j.mrpCol === false) miss.push('MRP location');
        if (j.wtgCol === false) miss.push('WTG ID');
        if (j.spCol === false) miss.push('SO number');
        if (miss.length) toast('Colonne non trovate: ' + miss.join(', ') + ' — non salvate', 'err');
        setRows([]);
        await loadRegistry();
  ...
  return (
    <>
      <TopBar warehouse={warehouse} onWarehouseChange={handleWarehouseChange} view={view} onViewChange={setView} />
      <div className="mx-auto max-w-205 p-4.5">
        {view === 'acq' && (
          <>
            <CaptureView queue={queue} onQueueChange={setQueue} processing={processing} progress={progress} onProcessAll={processAll} />
            <ReviewView rows={rows} registry={registry} onEdit={editRow} onSetType={setRowType} onDelRow={delRow} onAddManual={addManualRow} archDN={archDN} whLabels={WH_LABELS} />
            {showEmpty && (
              <div className={card}>
                <div className="px-2.5 py-7.5 text-center text-sm text-muted">
                  <FileText size={34} className="mx-auto mb-2.5" />
        ...
        {view === 'ana' && <AnagraficaView registry={registry} onSave={handleSaveAna} onDelete={handleDeleteAna} />}
        {view === 'arch' && <ArchivioView archive={archive} onRefresh={() => loadArchive()} loading={archiveLoading} />}
      </div>
      <FootBar visible={showFoot} onCancel={cancelRows} onConfirm={confirmRows} confirming={confirming} />
    </>
  );
}
